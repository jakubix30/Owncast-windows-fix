...
// https://chatgpt.com/c/6761f584-acfc-8012-b6fc-edc1f1d5e6eb
// edited changed func getRelativePathFromAbsolutePath
...
func GetRelativePathFromAbsolutePath(path string) string {
	...
	// Podziel ścieżkę na komponenty
	pathComponents := strings.Split(normalizedPath, "/")

	// Sprawdź liczbę komponentów
	if len(pathComponents) < 2 {
		log.Warnln("Invalid path passed to GetRelativePathFromAbsolutePath:", path)
		return path // Zwróć oryginalną ścieżkę w razie błędu
	}

	// Pobierz ostatnie dwa komponenty
	variant := pathComponents[len(pathComponents)-2]
	file := pathComponents[len(pathComponents)-1]

	// Zbuduj względną ścieżkę
...
func ValidatedFfmpegPath(ffmpegPath string) string {
	...
	log.Infoln("Sprawdzam lokalny plik ffmpeg:", localCopy)
	if err := VerifyFFMpegPath(localCopy); err == nil {
		log.Infoln("Znaleziono działającą lokalną kopię ffmpeg.")
		return localCopy
	} else {
		log.Warnln("Błąd podczas weryfikacji lokalnej kopii ffmpeg:", err)
	}

	// Jeśli lokalna kopia nie działa, szukamy w PATH
	log.Warnln("Próba znalezienia ffmpeg w PATH jako alternatywy...")
	var cmd *exec.Cmd
	if runtime.GOOS == "windows" {
	...
	if err != nil {
		log.Fatalln("Nie można znaleźć ffmpeg w PATH. Upewnij się, że lokalna kopia 'ffmpeg.exe' istnieje lub że ffmpeg jest dostępny globalnie.")
	}

	path := strings.TrimSpace(string(out))
	log.Infoln("Znaleziono ffmpeg w PATH:", path)
	if VerifyFFMpegPath(path) == nil {
		return path
	}

	log.Fatalln("Nie znaleziono działającego ffmpeg. Upewnij się, że lokalny plik lub PATH są poprawnie skonfigurowane.")
	return ""
}
...
func VerifyFFMpegPath(path string) error {
	...
	if stat.IsDir() {
		return errors.New("ffmpeg path cannot be a folder")
	}

	mode := stat.Mode()
	// Sprawdzamy, czy plik ma prawa wykonania
	if runtime.GOOS == "windows" {
		// Na Windowsie pliki .exe są domyślnie wykonywalne, więc ignorujemy ten krok
		if !strings.HasSuffix(strings.ToLower(path), ".exe") {
			return errors.New("ffmpeg path does not point to an executable file")
		}
	} else {
		// Na innych systemach upewniamy się, że plik jest wykonywalny
		if mode&0o111 == 0 {
